...
import 'package:sixvalley_vendor_app/provider/menu_provider.dart';
...
class _NavBarScreenState extends State<NavBarScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<MenuController>(builder: (context,menuController,_) {
      return Scaffold(
      ...
      child: Column(
        ...
        children: [
          SizedBox(height: Dimensions.NAVBAR_ICON_SIZE, width: Dimensions.NAVBAR_ICON_SIZE,
            child: Image.asset(icon, fit: BoxFit.contain, color: Provider.of<MenuController>(context, listen: false).currentTab == selectIndex
                  ? Theme.of(context).primaryColor : ColorResources.nevDefaultColor,
            ),
          ),
          const SizedBox(height: 6.0),
          Text(name, style: TextStyle(color: Provider.of<MenuController>(context, listen: false).currentTab == selectIndex
